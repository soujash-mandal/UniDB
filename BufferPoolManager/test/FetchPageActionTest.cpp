...
public:
  ...
  void Remove(PageId pageId) override {
    removedPages.push_back(pageId);
  }
...
protected:
  ...
  void SetUp() override {
    action = std::make_unique<FetchPageAction>(
        bufferPool,
        readPagePort,
        writePagePort,
        evictionPolicy);
  ...
  // --------------------------------------------------------------------------
  void fillBufferPool() {
    for (uint32_t frameId = 0;
         frameId < bufferPool.size();
         ++frameId) {
  ...
  // --------------------------------------------------------------------------
  Frame &fillBufferPoolWithVictim(uint32_t victimFrameId,
                                  PageId victimPageId) {
    fillBufferPool();

    Frame &victimFrame =
        bufferPool.getFrame(victimFrameId);
...
TEST_F(FetchPageActionTest,
       ReturnsExistingPageWithoutReadingFromDisk) {
...
TEST_F(FetchPageActionTest,
       ExistingPageBecomesNonEvictable) {
  Frame &frame = bufferPool.getFrame(0);
  ...
  EXPECT_TRUE(
      evictionPolicy.wasMarkedNonEvictable(42));
}

TEST_F(FetchPageActionTest,
       ExistingPageIsReturnedFromCorrectFrame) {
...
TEST_F(FetchPageActionTest,
       RepeatedFetchRecordsEveryAccess) {
  Frame &frame = bufferPool.getFrame(0);
...
TEST_F(FetchPageActionTest,
       EmptyFrameIsInitializedCorrectly) {
  action->execute(42);
...
TEST_F(FetchPageActionTest,
       CallsEvictionPolicyWhenBufferPoolIsFull) {
  fillBufferPool();
...
TEST_F(FetchPageActionTest,
       EvictsCleanPageWithoutWritingIt) {
  Frame &victimFrame =
...
TEST_F(FetchPageActionTest,
  ...
  EXPECT_FALSE(
      writePagePort.wasWritten(10));

  EXPECT_EQ(
      writePagePort.writeCount(),
      0);
}
...
TEST_F(FetchPageActionTest,
       DirtyVictimIsWrittenBeforeReuse) {
  Frame &victimFrame =
      fillBufferPoolWithVictim(0, 10);
...
TEST_F(FetchPageActionTest,
       DirtyVictimIsCleanedBeforeReuse) {
  Frame &victimFrame =
      fillBufferPoolWithVictim(0, 10);
  ...
  EXPECT_EQ(
      victimFrame.getPageId(),
      42);

  EXPECT_TRUE(
      victimFrame.isOccupied());
...
TEST_F(FetchPageActionTest,
       DirtyVictimIsWrittenExactlyOnce) {
  Frame &victimFrame =
      fillBufferPoolWithVictim(0, 10);
...
TEST_F(FetchPageActionTest,
       RemovesVictimFromEvictionPolicy) {
  fillBufferPoolWithVictim(0, 10);
  ...
  ASSERT_EQ(
      evictionPolicy.removedPages.size(),
      1);

  EXPECT_EQ(
      evictionPolicy.removedPages[0],
      10);
}

TEST_F(FetchPageActionTest,
       NewPageRecordsAccessAfterEviction) {
  fillBufferPoolWithVictim(0, 10);
  ...
  EXPECT_EQ(
      evictionPolicy.accessCount(42),
      1);
}

TEST_F(FetchPageActionTest,
       NewPageBecomesNonEvictableAfterEviction) {
  fillBufferPoolWithVictim(0, 10);
...
TEST_F(FetchPageActionTest,
       RequestedPageReusesVictimFrame) {
  Frame &victimFrame =
      fillBufferPoolWithVictim(0, 10);
...
TEST_F(FetchPageActionTest,
       CorrectVictimFrameIsReused) {
  Frame &frame0 =
      fillBufferPoolWithVictim(0, 10);

  Frame &frame1 =
      bufferPool.getFrame(1);
...
TEST_F(FetchPageActionTest,
       OtherFramesRemainUnchangedWhenVictimIsReused) {
  Frame &victimFrame =
      fillBufferPoolWithVictim(0, 10);
...
TEST_F(FetchPageActionTest,
  ...
  EXPECT_EQ(
      victimFrame.getPinCount(),
      1);
}

TEST_F(FetchPageActionTest,
       NewlyLoadedPageStartsClean) {
  Frame &victimFrame =
      fillBufferPoolWithVictim(0, 10);
...
TEST_F(FetchPageActionTest,
       ThrowsIfVictimIsNotInBufferPool) {
  fillBufferPool();
...
TEST_F(FetchPageActionTest,
       ExistingPageIsFoundBeforeEviction) {
  fillBufferPool();
  ...
  EXPECT_TRUE(
      evictionPolicy.removedPages.empty());

  EXPECT_EQ(
      existingFrame.getPinCount(),
      1);
}
...
TEST_F(FetchPageActionTest,
       CompleteDirtyVictimReplacementFlow) {
  Frame &victimFrame =
      fillBufferPoolWithVictim(0, 10);
